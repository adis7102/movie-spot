...
const Popular: NextPage<Props> = (props) => {
  ...
  const handleLoadMore = async (page: number) => {
    const resMovieList = await fetch(
      `${BASE_URL}/popular?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${
        page + 1
  ...
  return (
    <div>
      ...
      <Container fluid className="movie-list">
        <Row>
          {(popularStateData?.movieList || []).map(
            (item: MovieListItem, index) => (
              <Col className="movie-list-item-col" key={index} xs={12} md={4} lg={3} xxl={2}>
          ...
          {(popularStateData?.movieList || []).map(
            (item: MovieListItem, index) => (
              <Fragment key={index}>
                ...
                {index + 1 === (popularStateData?.movieList || []).length && (
...
export const getServerSideProps: GetServerSideProps =
  wrapper.getServerSideProps((store) => async () => {
    ...
    for (let i: number = 1; i <= 5; i++) {
      urls.push(
        `${BASE_URL}/popular?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${i}&with_watch_monetization_types=flatrate`
      );
    }
